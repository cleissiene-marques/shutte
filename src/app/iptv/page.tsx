...
export const metadata: Metadata = {
  title: "Ranking de IPTV por Estado: Compare e Teste Grátis",
  description: "Ranking independente de provedores de IPTV separado por estado: estabilidade, suporte e preço, com teste grátis sem cartão de crédito, para assinantes de todas as regiões do Brasil.",
    ...
    <>
      <section className="hero page-hero" aria-labelledby="hero-heading">
        <div className="container">
          <div className="label">Ranking por estado</div>
          <h1 className="hero-title" id="hero-heading">
            O <span className="hl-1">ranking de IPTV</span> em cada estado do Brasil
          </h1>
          <p className="hero-desc">
            O ranking da Shutte é nacional e todos os provedores avaliados oferecem teste grátis, sem cartão de crédito. Escolha seu estado para ver o ranking com contexto local, preços e perguntas frequentes específicas.
          </p>
        </div>
      </section>
              ...
              <div className="blog-grid" style={{ marginTop: "48px" }}>
                {statesInRegion.map((state) => (
                  <Link href={`/iptv/${state.slug}/`} className="card blog-card animate-on-scroll" key={state.slug}>
                    ...
                    <h3>{state.name}</h3>
                    <p>Capital: {state.capital} — ver ranking de IPTV {state.em} →</p>
                  </Link>
                ))}
              </div>
            </div>
      ...
      <section className="section" aria-label="Contato">
        <div className="container">
          <p className="blog-notice">
            Não encontrou seu estado ou tem dúvida sobre o ranking?{" "}
